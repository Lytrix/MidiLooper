...
Live edit RAM uses a single **`EditSession`** on **EditManager** with **`EditSessionType`**
(`Loop`, `Note`, `ControlChange`). Stored **editPass** rows use **`EditPassType`** (`Note`,
`ControlChange`, `Audio`). Shipped in **edit-session-state** (archived 2026-06-23).
