...
**ChangeLength** commit during **NoteEditSession** SHALL rematerialize the moving note end into
**passes.editPasses** and session store so HITL checkpoints match live preview. Shipped Track A+B
on capture `203729` (archived 2026-06-20). Track C insert/reorder deferred.

## Requirements

### Requirement: ChangeLength commit updates persisted moving note end

The system SHALL persist the moving note's new end tick in **passes.editPasses** on **ChangeLength** commit so **LoopPasses::materialize** replay on **NoteEditSession.store** matches the live end tick previewed before commit.

#### Scenario: Post-commit reconstruction shows lengthened M0

- **WHEN** the user lengthens M0 on the standard edit fixture and commits **ChangeLength** with `newEnd` at the long fixture step
...
Materialized inventory from **passes** replay SHALL match the live **NoteEditSession.store** at each HITL checkpoint after **ChangeLength** commit and subsequent overlap edits in the same session.
...
The change SHALL document root cause and capture id in BUG.md patch history before archive.
