...
Before **saveNoteEditPass** at fader-1 reselect or commit boundary, the system SHALL resolve **overlapNotes** into the store for impacted **NoteRef**s only, emit **EditChange** entries only for notes whose committed state differs from **commitBaseline** / baseline map, and rebuild the session store via **LoopPasses::materialize** overlay.
