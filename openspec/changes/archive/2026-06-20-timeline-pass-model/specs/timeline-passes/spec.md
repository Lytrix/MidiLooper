...
**`passes.editPasses[]`**. An **editPass** SHALL hold **editPassId**, **EditPassKind**, batch index,
and an **EditChange** list. The legacy struct name **Edit** SHALL NOT remain in product code.

#### Scenario: saveNoteEditPass after delete
...
entries in **`passes[]`**, with chunk-backed storage. Live capture SHALL use **Capture** until
**commitRecordPass** or **commitOverdubPass** promotes the **pendingCapturePass** into **`passes[]`**.
...
- **AND** **Capture** is cleared for the next session

## REMOVED Requirements

### Requirement: Take vocabulary in loop storage

**Reason:** Superseded by **recordPass** / **overdubPass** / **`passes[]`**.  
**Migration:** `timeline-pass-model` tasks; SD dual-read.

### Requirement: TakeCommitted global undo

**Reason:** Replaced by **RecordPassAdded** and **OverdubPassAdded**.  
**Migration:** **GlobalUndoStack** enum + **TrackUndo**.

### Requirement: Edit struct name in passes storage

**Reason:** Renamed **Edit** → **editPass** in **`passes.editPasses[]`**.  
**Migration:** **saveEditPass**, **editPassId**, **disableEditPasses**; **EditChange** unchanged.

### Requirement: NoteEditSessionCommitted global undo

**Reason:** Replaced by **NoteEditPassClosed**.  
**Migration:** **GlobalUndoStack** enum + **TrackUndo**.
