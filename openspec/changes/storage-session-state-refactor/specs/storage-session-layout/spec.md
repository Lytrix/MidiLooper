...
- **THEN** `resetRevisionLoadJobState`, `dispatchStagedRevisionLoad`, `clearRevisionLoadPromptAndPipelineState`, and `stepRevisionLoadJob` are defined there
...
- **THEN** it calls `StorageManagerInternal::dispatchStagedRevisionLoad` (or renamed equivalent)
