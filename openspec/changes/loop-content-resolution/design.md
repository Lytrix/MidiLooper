...
3. If all three gates pass: **6A** idle display range via `resolveWindow` (old path oracle) → **6B** commit marks affected ranges only → **6C** overdub source from prepared LCR with 3b copy fallback (consume-when-ready). **6D.1** one-vector mutation **FAIL**. **6D.2** split **PASS**. **6D.3** repeated overdub commits **PASS** (native; not live incremental LCR). Production architecture gate may now be considered. Long-loop playback / NOTE_EDIT last. Never construct/sort/checkpoint/resolve LCR on overdub start/stop to open the source view. **6A PASS** [`185931`](../../../captures/session_20260815_185931.log). **6B PASS** [`192334`](../../../captures/session_20260815_192334.log). **6C** native landed.
