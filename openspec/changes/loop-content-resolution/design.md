...
9. **`TickIndex` owns event-range lookup.** Contract: `tick ∈ [begin, end)` → Active `(passId, eventIndex)`. 5.17d device IndexCommit is flat A ([`161355`](../../../captures/session_20260815_161355.log) `iapp=4819607` `isort=27415` `win=13971`). Native `commitCapturePass` still fills `byTick`. Do not collapse this index into `spanBoundaries`.
10. **Determinism.** Same active history ⇒ same answer, independent of cache, chunks, checkpoint density, or prior order.
