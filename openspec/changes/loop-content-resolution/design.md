...
- `pair` `DFRAME` 1.277 s [`170024`](../../../captures/session_20260815_170024.log) — **5.18**. Contracts pinned from code: `byNoteId` last-wins `NoteId → {pass, on, off}`; `openOnByPitch` per-pass LIFO keyed by pitch only. Open: which structure produces the stall, then minimum representation for **that** query. Plan: [`loop_content_resolution_pair_index_refinement.md`](../../../docs/Plans/loop_content_resolution_pair_index_refinement.md). Not 5.7c.
