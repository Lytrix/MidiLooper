...
9. **`TickIndex` owns event-range lookup.** Contract: `tick ∈ [begin, end)` → Active `(passId, eventIndex)`. Flat A (`tickEvents`, C-order append + `stable_sort` by tick). 5.17d device [`161355`](../../../captures/session_20260815_161355.log). 5.17e removed `byTick`. Do not collapse this index into `spanBoundaries`.
