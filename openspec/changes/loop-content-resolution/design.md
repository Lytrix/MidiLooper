...
9. **`TickIndex` owns event-range lookup.** Contract: `tick ∈ [begin, end)` → Active `(passId, eventIndex)`. 5.17a–c native pick **A** (`TickEventEntry[]`, `stable_sort` by tick). Production `byTick` stays C until 5.17d. Do not collapse this index into `spanBoundaries`.
