...
3. If all three gates pass: **6A** idle display range via `resolveWindow` (old path oracle) → **6B** commit marks affected ranges only → **6C** overdub source from prepared LCR with 3b copy fallback (consume-when-ready). **6D.1 FAIL.** **6D.2/6D.3 PASS.** **6D.4 architecture approved; firmware not authorized.** Long-loop playback / NOTE_EDIT last. Never construct/sort/checkpoint/resolve LCR on the overdub button / source-view path. **6A PASS** [`185931`](../../../captures/session_20260815_185931.log). **6B PASS** [`192334`](../../../captures/session_20260815_192334.log). **6C** native landed.
