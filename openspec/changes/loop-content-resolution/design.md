...
3. If all three gates pass: **6A** idle display range via `resolveWindow` (old path oracle) → **6B** commit marks affected ranges only → **6C** overdub source from prepared LCR with 3b copy fallback (consume-when-ready). **6D** incremental post-commit maintenance is the always-ready path — investigation before firmware. Long-loop playback / NOTE_EDIT last. Never construct/sort/checkpoint/resolve LCR on overdub start/stop. **6A PASS** [`185931`](../../../captures/session_20260815_185931.log). **6B PASS** [`192334`](../../../captures/session_20260815_192334.log). **6C** native landed.
