...
15. **Device 5.1/5.2 complete; Stage 6 consume-only + 6A/6B/6C closed; 6D overdub-query slice closed.** 5.15–5.18 **FROZEN**. **5.1 PASS** [`173842`](../../../captures/session_20260815_173842.log). **5.2 PASS** [`180624`](../../../captures/session_20260815_180624.log) `begin_capture` 10050 µs. Overdub **button / source-view open** MUST NOT synchronously construct, sort, checkpoint, or resolve LCR state — consume already-prepared derived state only. **Approved:** commit-site delta sort + restamp at `finalizeCommitSideEffects` when a prepared index exists. Prepared validity may come from that update, not only `deviceGateComplete`. LCR is the producer of prepared derived state, not a replacement for `overdubSourceView`. Keep 3b copy. `< 3 ms` is consume of prepared state (3b 2214 µs copy), not a promise that LCR is 2.2 ms; `< 50 ms` hard gate. **6C closed** [`205928`](../../../captures/session_20260815_205928.log) / [`210508`](../../../captures/session_20260815_210508.log) (`6c` consume slower than 3b; 3b stays). **6D.1 FAIL.** **6D.2/6D.3 PASS.** **6D.4 landed** + HITL PASS same captures (not all of LCR live). A/B rejected. Do not persist. Do not call resolution from `handleMidiInput`. No B.
16. **Editor consume (DEC-037 amendment 2026-08-16; architecture PASS; stages PASS WITH AMENDMENTS; firmware not authorized).** `selectedTick` is the Editor query origin. Idle `visualCache` stays piano-roll paint. Select is a bounded `tickEvents` / `spanBoundaries` neighborhood — **not** `resolveState(selectedTick)`. Overlap is indexed identities then identity-bounded `appendNoteEvents`; `resolveWindow` must not return a reconstructed whole-loop `DisplayNote` vector. Prepared hit is analysis authority; miss is legacy compatibility only. Participating notes are found after mover stop plus one `DisplayManager::update` as **trigger**; data source is LCR ∪ overlay, not `visualCache` rebuild. Leavers `RestoreNote` (not DEC-030 sticky overlap end-of-participation); new intersections join Active. Remaining Active overlay is the session delta: next select/overlap is prepared LCR ∪ overlay (6D.4 analog). Do not insert live geometry into LCR (Path B). Audition overlays settled session `NoteId`s onto the playback window around `currentTick` — not a whole-loop `sessionMidiEvents()` replace. Stages: 1 select work-shape → 2 overlap identities → 3 consume + legacy miss → 4a open / 4b first select / 4c playback overlay → 5 device. Do not redesign `rematerializeEditView` / `materializeToEventVector` / `EditSession.store` in that migration. Same 6.0 consume-only rule on NOTE_EDIT open / fader. **Work identity:** [`note_edit_hydrate_enhancement.md`](../../../docs/Plans/note_edit_hydrate_enhancement.md) (not remaining 6.4 firmware in this change). Architecture: [`note_edit_selectedtick_lcr_resolution_architecture.md`](../../../docs/Plans/note_edit_selectedtick_lcr_resolution_architecture.md).
...
| Per-bar `presentAt` copies the resolved loop ([`225351`](../../../captures/session_20260814_225351.log) Critical / reboot) | Sparse stride; abort under pressure; checkpoint must not scale with bars × notes |
...
3. If all three gates pass: **6A** idle display range via `resolveWindow` (old path oracle) → **6B** commit marks affected ranges only → **6C** overdub source from prepared LCR with 3b copy fallback (consume-when-ready). **6D.1 FAIL.** **6D.2/6D.3 PASS.** **6D.4 landed.** **6A PASS** [`185931`](../../../captures/session_20260815_185931.log). **6B PASS** [`192334`](../../../captures/session_20260815_192334.log). **6C closed** [`205928`](../../../captures/session_20260815_205928.log) / [`210508`](../../../captures/session_20260815_210508.log). **6D HITL PASS** same captures (overdub-query slice only). Editor consume is **not** remaining firmware in this change — [`note_edit_hydrate_enhancement.md`](../../../docs/Plans/note_edit_hydrate_enhancement.md). Playback gather is **not** remaining firmware in this change — [`playback_gather_lcr_consume_enhancement.md`](../../../docs/Plans/playback_gather_lcr_consume_enhancement.md). Never construct/sort/checkpoint/resolve LCR on the overdub button / source-view path or NOTE_EDIT open / fader.
...
- Pair leftover **closed** [`173842`](../../../captures/session_20260815_173842.log). **5.1 PASS** same capture. **5.2 PASS** [`180624`](../../../captures/session_20260815_180624.log) `begin_capture` 10050 µs. **6A PASS** [`185931`](../../../captures/session_20260815_185931.log) `match=1`. **6B PASS** [`192334`](../../../captures/session_20260815_192334.log). **6C closed** [`205928`](../../../captures/session_20260815_205928.log) / [`210508`](../../../captures/session_20260815_210508.log) (3b stays). **6D.1 FAIL.** **6D.2/6D.3 PASS.** **6D.4 HITL PASS** same captures. Not all of LCR. A/B rejected. [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md). After 6C device: MIDI Input Gap > 50 ms in [`192334`](../../../captures/session_20260815_192334.log) (`midi_gap` 135 / 119 / 138 ms, `clockrate` 47). Playback gather moved: [`playback_gather_lcr_consume_enhancement.md`](../../../docs/Plans/playback_gather_lcr_consume_enhancement.md).
