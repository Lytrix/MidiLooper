...
15. **Device 5.1/5.2 order.** 5.15 complete. 5.16c `prep` PASS. 5.17 complete. 5.7c channel index PASS [`170024`](../../../captures/session_20260815_170024.log). 5.7 leftover is `pair` `DFRAME` 1.277 s — separate owner, not a derived-index swap. Do not persist. Do not put resolution on overdub or MIDI. Do not start Stage 6 / 5.1 until that leftover is decided.
...
| Per-entry PSRAM map/multimap insert on derived indexes | DEC-037 derived-index storage invariant; 5.15 / 5.17 / 5.7c measured. `pair` / `byNoteId` stay a separate pass |
...
- `pair` `DFRAME` 1.277 s [`170024`](../../../captures/session_20260815_170024.log) — separate measurement/design pass. Pin `byNoteId` / `openOnByPitch` query contracts before any flat swap. Not 5.7c.
