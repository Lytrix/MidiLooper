...
- [ ] 6B Commit invalidation: overdub stop commits, marks **only affected ranges**, returns. No materialize, no whole-loop reconstruct, no `VCACHE,full`. Separate stop-entry / commit / return-to-MIDI (bounded) from idle prep / repaint (sliced) / eventual consistency (async). After 6A. **Native shipped** (`test_mark_affected_display_cache_ranges_dirties_sparse_bars`; `pio test -e native` 1198/1198). Device gate: no `VCACHE,stale_all` / no shrinking `adopt_partial` on 139-bar overdub stop; expect `VCACHE,stale_range` with `dcnt` ≪ 139.
