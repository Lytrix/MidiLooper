...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. [`161355`](../../../captures/session_20260815_161355.log) complete `hist=2394` `walk=0` `reb=6865156` `st=3108` `win=13971` `iapp=4819607` `isort=27415`. 5.17d **PASS** — no LCR `idle_maint` remainder. Leftover: `DFRAME` gaps **1.3–1.58 s** during `spans`. Do not treat `reb=6.87 s` as the fail.
...
- [ ] 5.17e Drop `byTick` from `commitCapturePass` / `indexCapturePassEventRange` only if asked. Device gate already uses A. No B. No A2. No `recon`. No `pair`.
