...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. [`151450`](../../../captures/session_20260815_151450.log) complete `hist=2394` `walk=0` `reb=13851136` `app=5490706` `sort=10202` `st=13045`. 5.15 **complete** — map emplace gone. 5.16a: no capture line shows a 1.4 s uninterrupted slice (`DFRAME` duration stays ~25 ms; gaps are late 1 Hz logs). Remaining written-bar leftovers: `prep` **182 ms** one-shot; `idx`/`pair` pass-0 slices 63–83 ms; `DFRAME` gaps >1 s in `idx`/`pair`/`proj`/`spans`. Do not treat `reb=13.85 s` as the fail.
...
- [ ] 5.16b Do not add allocation telemetry unless a later slice needs it to pick an owner. First implementation candidate after this audit: slice `prep` (not started). Do not start `byTick` swap, A2, B, 5.1/5.2/6.x.
