...
- [ ] 6C Prepared LCR range → `overdubSourceView`. Native landed: `establishOverdubSourceView` consumes `tryResolvePreparedWindow` (miss/stamp mismatch never rebuilds); 3b visual-cache copy stays fallback. Device score next against **2214 µs**, not 5.2 **10050 µs**. Look for `DIAG,lcr,6c`.
