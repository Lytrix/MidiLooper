...
- [ ] **6D** Incremental post-commit LCR maintenance — **investigation only** (native measurement). [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md). A (re-arm STOPPED cold-build) and B (slice 30–60 s full-history build during PLAYING) **rejected**. Do not implement firmware until native scaling is `O(new events + affected checkpoints)`. PLAYING LCR slices are a scheduling/admission change — DEC-037 amendment before code.
