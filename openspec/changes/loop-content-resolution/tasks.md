...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size accepted. [`141630`](../../../captures/session_20260815_141630.log) complete `hist=2394`; reconstruct walk PASS; leftover project+dedup **280 ms**. Re-measure after 5.14 (`proj` climbing `ev=` / `notes=`, then one `dedup`).
