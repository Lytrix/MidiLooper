...
- [x] 5.4 Sparse `presentAt`: keep `spans` + `startsByTick`; device stride **8 bars** (`kDeviceCheckpointBarStride`). Native Stage 7/8 stay at 1 bar. Same `resolveState` answers (`test_stage7_sparse_checkpoints_agree_with_dense`).
...
- [x] 6A.1 Native: `resolveWindow` unique keeps same-tick NoteOffs (`noteId` is ON-only); skip `appendNoteEvents` when the target ON is already in the window. `test_resolve_window_delete_keeps_same_pitch_same_end_note` from [`024806`](../../../captures/session_20260816_024806.log) (Delete 1034 keeps 1422). Device `6a` `match=` is LCR vs native. **HITL PASS** [`025651`](../../../captures/session_20260816_025651.log) `ev=126 notes=57 miss=0 extra=0 match=1 pmatch=1`. `gmatch=0` is gather remaps.
...
- [x] 6C Prepared LCR range → `overdubSourceView`. Consume-when-ready **closed**. Native landed. Device: [`205928`](../../../captures/session_20260815_205928.log) `DIAG,lcr,6c` `begin_capture` **37747 µs**; [`210508`](../../../captures/session_20260815_210508.log) restamp hit **31128 µs**, third consume **312636 µs**. [`194643`](../../../captures/session_20260815_194643.log) RING dropped the first `6c` — later captures scored it. 3b copy stays the fast path ([`045556`](../../../captures/session_20260814_045556.log) **2214 µs**). LCR window reconstruct on the button is not a 3b replacement. Do not treat 6C as “LCR beat 3b.”
- [x] **6D** Incremental post-commit **overdub-query** index **closed** (not all of LCR live). **6D.1 FAIL.** **6D.2/6D.3 native PASS.** **6D.4 landed** + HITL PASS: [`205928`](../../../captures/session_20260815_205928.log) PLAYING publish restamp (`6c` without a second `deviceGateComplete`); [`210508`](../../../captures/session_20260815_210508.log) undo miss → 3b `begin_capture` **120 µs**, then PLAYING publish `6c`. [`PREFLIGHT.md`](PREFLIGHT.md). [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md) § 6D.4. A/B **rejected**.
...
- [ ] 6.3 **Moved.** Long-loop playback gather → `resolveWindow` / `ResolvedEvent` is **not** remaining LCR production-swap firmware. Work identity: [`playback_gather_lcr_consume_enhancement.md`](../../../docs/Plans/playback_gather_lcr_consume_enhancement.md). DEC-037 amendment 2026-08-17. Do not start from this change’s remaining 6.x tasks.
...
- [x] **038.1 live +1** After first sealed wrap, `overdubSessionUndoDepth` = cursor + live store; undo live first. Next S still skipped. HITL PASS [`011413`](../../../captures/session_20260816_011413.log).
...
- Playback gather firmware — [`playback_gather_lcr_consume_enhancement.md`](../../../docs/Plans/playback_gather_lcr_consume_enhancement.md)
