...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. 5.17d **PASS**. 5.7a reserve **PASS** [`163942`](../../../captures/session_20260815_163942.log). **5.7b device FAIL** [`164922`](../../../captures/session_20260815_164922.log). **5.7c native** — flat channel lookup; device remasure owed. First `spans` slice must not contain a multi-second prerequisite build.
...
- [x] 5.7c Native: `channelByNoteId` is `{noteId, channel}[]` — C-order NOTE_ON append, `stable_sort` by `noteId`, first-wins unique, `lower_bound` lookup. Device sequences `chan` / `csort` before `spans`. Complete line `capp=` / `csort=`. `test_stage57c_channel_index_*` PASS. Device remasure owed. Do not rewrite `pair` / `recon`.
