...
- [ ] **6D** Incremental post-commit overdub-query index — investigation. **6D.1 native instrument landed.** Correctness PASS (append+merge = cold `commitCapturePass` / materialize oracle). Scaling **FAIL** for full `tickEvents` sort **and** ordered merge: `index_order_us` grows with `history_events` at fixed `commit_delta_events` (sort 602→2624 µs, merge 46→174 µs at 8192→32768; both < 50 ms). Mutation contract not established. Production untouched. [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md). A/B **rejected**. 6.0 unchanged.
