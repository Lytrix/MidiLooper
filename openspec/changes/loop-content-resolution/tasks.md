...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. [`151450`](../../../captures/session_20260815_151450.log) complete `hist=2394` `walk=0` `reb=13851136` `app=5490706` `sort=10202` `st=13045`. 5.15c `spans` append **PASS** vs [`143009`](../../../captures/session_20260815_143009.log) map emplace 224–413 ms (`loop_rem` gone in `spans`; 5 s `idle_maint` 33–60 ms; `DFRAME` duration ~25 ms, gaps **1.4–2.2 s** was 8.9–13.1 s). Still over the written bar: `prep` **182 ms**; `idx`/`pair` **50–83 ms** (`byTick`); `DFRAME` gaps in `spans` still >1 s.
