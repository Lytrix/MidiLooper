...
- [ ] 6A Prepared display range (idle): one dirty range via `resolveWindow` → display projection instead of `CommittedEventRange` → `reconstructDisplayNotes`. Keep old path as oracle. Firmware: keep `TickIndex` after `deviceGateComplete`; `rebuildVisualCacheIdleSlice` consumes `tryResolvePreparedWindow` (stamp = `playbackRevision`); miss falls back to gather. Native oracle tests added. Device measure owed: `DIAG,lcr,6a,…,match=`, `midi_gap`, `DFRAME`. Do not wire LCR into overdub.
- [ ] 6B Commit invalidation: overdub stop commits, marks **only affected ranges**, returns. No materialize, no whole-loop reconstruct, no `VCACHE,full`. Separate stop-entry / commit / return-to-MIDI (bounded) from idle prep / repaint (sliced) / eventual consistency (async). After 6A.
- [ ] 6C Prepared LCR range → `overdubSourceView`. Only after 6A/6B. 3b visual-cache copy stays as fallback. Score `begin_capture` against **2214 µs**, not 5.2 **10050 µs**.
