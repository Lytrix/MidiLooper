...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. 5.17d **PASS**. 5.7a reserve **PASS** [`163942`](../../../captures/session_20260815_163942.log). 5.7b native **PASS**; device remasure owed.
...
- [x] 5.7b Native: `channelByNoteId` from every NOTE_ON in `resolved` (first wins). Open note whose OFF is outside the current 8 events still pairs (`test_stage57_recon_keeps_open_note_across_event_slice` / `test_stage57_pair_keeps_open_note_across_event_slice`) and still gets channel (`test_stage57_span_channel_uses_full_resolved_not_note_slice`). Device remasure owed. Do not rewrite `pair` / `recon`.
