...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size accepted. [`143009`](../../../captures/session_20260815_143009.log) complete `hist=2394` `reb=104359946` `walk=0`. 5.14 project **PASS** (`proj` `ev` 8→2304, `idle_maint` 32–64 ms; `dedup` **60 ms**). Latency **FAIL**: span emplace **224–413 ms**; `DFRAME` **5.8–13.1 s** in `spans`; `prep` **182 ms**.
