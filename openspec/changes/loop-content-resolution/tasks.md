...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. 5.17d **PASS**. 5.7a reserve **PASS**. **5.7c device PASS** [`170024`](../../../captures/session_20260815_170024.log) `capp=12373` `csort=1779`; no `idle_maint` `loop_rem`; `chan`/`spans` `DFRAME` 0.933–0.965 s. **5.7 leftover:** `pair` `DFRAME` 1.277 s (different owner).
...
- [x] 5.7c Native PASS / **device PASS** [`170024`](../../../captures/session_20260815_170024.log): flat `{noteId, channel}` append + `stable_sort` + first-wins unique. `dedup` → `chan` (11.55 ms) → `csort` → `spans` (22.63 ms). `capp=12373` `csort=1779`. `walk=0`. Do not rewrite `pair` / `recon`.
