...
- [ ] 5.7 Measure selected-loop idle gate on a loop **>63 bars**: `DIAG,lcr` or explicit skip; no `idle_maint` ~50 ms; no 1 s `DFRAME` gaps. Size **PASS**. 5.17d **PASS** — no LCR `idle_maint` remainder. 5.7a reserve **PASS** [`163942`](../../../captures/session_20260815_163942.log) `app=2349` `iapp=202981`; `idx` 1310→1212 events/s. Leftover: `spans` 930→169 notes/s; `DFRAME` 0.992→1.195 s. Remaining owner: `channelForNoteId` in `appendSpansFromNotes`.
- [x] 5.7a Native: `appendSpansFromNotes` reserves `2 * notes.size()`; `appendTickEventEntries` reserves remaining events in the pass. `test_stage57_*` PASS. Device [`163942`](../../../captures/session_20260815_163942.log) reserve **PASS** (`app=2349` `iapp=202981`). Do not rewrite `channelForNoteId` in this slice.
