...
- [x] 5.7 Representation on the >63-bar idle gate **closed** (5.7c FROZEN [`170024`](../../../captures/session_20260815_170024.log)): `spanBoundaries` / `tickEvents` / channel lookup flat; `walk=0`; no `idle_maint` `loop_rem` on those paths; `chan`/`spans` `DFRAME` 0.933–0.965 s. Whole-gate `DFRAME` ≤ 1 s is **not** closed — `pair` 1.277 s is 5.18.
...
- [ ] 5.18 Pair index — design only until asked. Plan: [`loop_content_resolution_pair_index_refinement.md`](../../../docs/Plans/loop_content_resolution_pair_index_refinement.md). Pin `byNoteId` / `openOnByPitch` contracts from code; measure which produces the 1.277 s `DFRAME`; do not flatten from container type. No 5.1. No B. No `recon`.
