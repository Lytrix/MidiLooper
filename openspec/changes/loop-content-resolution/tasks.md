...
- [ ] **6D** Incremental post-commit overdub-query index — investigation. **6D.1 first:** one committed `OverdubPass` updates `capturePasses` + `tickEvents` + stamp only. Do not rebuild all of LCR. Do not solve edits, undo, disabled passes, checkpoints, or `byNoteId`. [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md). A/B **rejected**. 6.0 unchanged. Native before firmware.
