...
- [ ] **6D** Incremental post-commit overdub-query index — investigation. **6D.1 first (native only):** one committed `OverdubPass` updates `capturePasses` + `tickEvents` + stamp in `test_loop_content_resolution`. Measure `index_order_us` against **both** `history_events` and `commit_delta_events` at ≥2 history sizes, same delta. Full `tickEvents` sort that grows with history **fails** even if every sample is < 50 ms. Production commit/idle/overdub/`preparedWindowReady` **untouched** until that contract exists. Do not rebuild all of LCR. Do not solve edits, undo, checkpoints, or `byNoteId`. [`loop_content_resolution_incremental_commit_maintenance_refinement.md`](../../../docs/Plans/loop_content_resolution_incremental_commit_maintenance_refinement.md). A/B **rejected**. 6.0 unchanged.
