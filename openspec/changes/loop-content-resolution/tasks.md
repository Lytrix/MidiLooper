...
- [x] 5.18b Native: last-wins flat `byNoteId` (append + `stable_sort` + unique keep-last). Device `nsort` after all pair slices, before `isort`. Do not flatten `openOnByPitch`. Device remasure next. No 5.1. No B. No `recon`.
