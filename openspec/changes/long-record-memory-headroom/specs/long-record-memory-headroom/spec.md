...
Deferred save SHALL persist loop state in bounded incremental slices whose maximum temporary MIDI event buffer is no larger than one loop-event chunk (`LoopEventStoreConfig::CHUNK_CAPACITY`), including the undo-stack stage, so it can complete when free RAM2 is low.
...
- **THEN** each save slice yields control so MIDI clock, note-out, and playback are serviced
