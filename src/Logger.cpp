...
// By default, all categories enabled except MOVE_NOTES and MIDI_LED (verbose)
bool Logger::categoryEnabled[] = { true, true, true, true, true, true, true, false, false };
Logger logger;
...
void Logger::setCategoryEnabled(LogCategory category, bool enabled) {
  if (category >= CAT_GENERAL && category <= CAT_MIDI_LED) {
...
void Logger::printCategory(LogCategory category) {
  const char* categories[] = {"GEN", "STATE", "MIDI", "CLOCK", "TRACK", "BTN", "DISP", "MOVE", "LED"};
