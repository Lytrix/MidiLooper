    ...
    switch (job.loopFinalizeStage) {
        ...
        case DeferredLoopFinalizeStage::EpochCrcBody: {
            if (!job.loopFile) {
                ...
                if (!job.loopFile) {
                    Serial.println(
                        "[StorageManager] ERROR: Deferred save failed opening loop temp for epoch CRC");
                    return false;
                }
                const size_t fileSize = job.loopFile.size();
                if (fileSize < CurrentWorkspaceStorage::kEpochFileHeaderByteSize) {
                    job.loopFile.close();
                    Serial.println("[StorageManager] ERROR: Deferred save loop temp shorter than epoch header");
                    return false;
                }
                job.epochCrc = 0;
                ...
                if (!job.loopFile.seek(CurrentWorkspaceStorage::kEpochFileHeaderByteSize)) {
                    job.loopFile.close();
                    Serial.println("[StorageManager] ERROR: Deferred save failed seeking loop temp CRC body");
                    return false;
                }
            ...
            if (bytesRead <= 0) {
                job.loopFile.close();
                Serial.println("[StorageManager] ERROR: Deferred save failed reading loop temp CRC body");
                return false;
            }
        ...
        case DeferredLoopFinalizeStage::EpochCrcHeader:
            if (!CurrentWorkspaceStorage::writeEpochFileHeaderCrc(tempPath, job.epochCrc)) {
                Serial.println(
                    "[StorageManager] ERROR: Deferred save failed refreshing loop temp epoch CRC");
                return false;
            }
