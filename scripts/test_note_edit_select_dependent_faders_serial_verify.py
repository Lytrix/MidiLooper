...
class NoteEditSelectDependentFadersSerialVerifyTests(unittest.TestCase):
    def _note_changed_bundle(
        ...
        *,
        ...
        return [
            ...
            (
                f"[{t + 0.01:.3f}] #DBG outbound_ctx f2 anchor_tick=96 rel_tick=96 "
                f"loop_start=0 loop_len=1536 pb={f2_pb} expected_pb_rel={f2_pb} "
                f"step=6 f1_pb=0 slot=1 mode=SELECT_SYNC"
            ),
            (
                f"[{t + 0.02:.3f}] #DBG outbound_ctx f4 pitch={f4_pitch} "
                f"selected_idx={note_idx} mode=SELECT_SYNC"
            ),
            (
                f"[{t + 0.03:.3f}] #DBG select_motor_sync sent=1 note_idx={note_idx} "
                f"prior_note_idx=-1 reason=note_changed f2_pb={f2_pb} f4_cc={f4_pitch} "
                f"prior_f2_pb=0 prior_f4_cc=0 motor_value_changed=1"
            ),
            f"[{t + 0.04:.3f}] #DBG select_motor_sync mode=SELECT_SYNC sequence=timed_burst",
            f"[{t + 0.05:.3f}] #CAP,1,MO,224,14,{d1},{d2}",
            f"[{t + 0.06:.3f}] #CAP,1,MO,144,14,0,127",
            f"[{t + 0.061:.3f}] #CAP,1,MI,H,144,13,82,127",
            f"[{t + 0.07:.3f}] #CAP,1,MO,176,15,2,64",
            f"[{t + 0.08:.3f}] #CAP,1,MO,144,15,1,127",
            f"[{t + 0.081:.3f}] #CAP,1,MI,H,144,13,84,127",
            f"[{t + 0.09:.3f}] #CAP,1,MO,176,15,3,{f4_pitch}",
            f"[{t + 0.10:.3f}] #CAP,1,MO,144,15,2,127",
            f"[{t + 0.101:.3f}] #CAP,1,MI,H,144,13,86,127",
            f"[{t + 0.14:.3f}] #CAP,1,MI,H,224,14,{d1},{d2}",
            f"[{t + 0.15:.3f}] #CAP,1,MI,H,176,15,3,{f4_pitch}",
        ]
    ...
    def test_fail_when_f2_mo_value_mismatch(self) -> None:
        ...
        lines[5] = f"[1.050] #CAP,1,MO,224,14,{wrong_d1},{wrong_d2}"
