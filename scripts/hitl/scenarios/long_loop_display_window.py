...
def run_long_loop_display_window(args: object) -> int:
    ...
    try:
        ...
        print(f"[long-loop-display-hitl] select track {ns.track_number}")
        _send_short_press(
            out_port,
        ...
        print(f"[long-loop-display-hitl] record {ns.record_bars} bars")
        _send_short_press(
            out_port,
            note=RECORD_BUTTON_NOTE,
            channel_1based=CONTROL_CHANNEL_1BASED,
        ...
        if not _ensure_midi_clock(in_port, out_port, min_clocks=24, timeout_s=3.0, abort=abort):
            print("[long-loop-display-hitl] MIDI clock missing before record")
            return 1
