...
def verify_fader_select_dwell_gap(
    ...
    *,
        ...
        if not motor_changed:
            ...
            if len(result.dwell_gap_details) < 10:
                result.dwell_gap_details.append(
                    ...
                    f"within {motor_window_s * 1000:.0f}ms"
                )
        dwell_start = (t, slot, pitch)
    ...
    for i in range(1, len(dnte_events)):
        ...
        f2_hit = any(mt >= cur_t and mt <= window_end for mt, _pb in mo_f2_events)
        f4_hit = any(mt >= cur_t and mt <= window_end for mt, _cc in mo_f4_events)
        if not f2_hit and not f4_hit:
            result.dnte_motor_misses += 1
            if len(result.dnte_motor_details) < 10:
                result.dnte_motor_details.append(
                    f"DNTE sel {prev_idx}->{cur_idx} at {cur_t:.3f}s: no F2/F4 MO within "
