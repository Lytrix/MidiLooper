...
6. **State stays on `Track`** — move **method bodies** only; do **not** introduce `CaptureManager` / `PlaybackEngine` top-level classes without user approval ([NAMING.md](../00-authority/NAMING.md) § Module suffix).
