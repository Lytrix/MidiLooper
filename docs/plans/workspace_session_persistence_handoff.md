...
**Next work:** [`set_revision_persistence_architecture_enhancement.md`](set_revision_persistence_architecture_enhancement.md) — OpenSpec **`set-revision-persistence`** task **1.1** (metadata + catalog foundation).

**Superseded for new implementation:** flat SavedSet snapshots, auto-save-before-load, packed `loops.bin`, M3/M4 as written in this change. See `openspec/changes/set-revision-persistence/` and `workspace-session-persistence/SUPERSEDED.md`.
...
| Deferred FSM retarget | `StorageManager.cpp` → `MidiLooper/current/workspace.bin` + `loop_TT_SS.bin` |
...
| Change | Status | Apply |
|--------|--------|-------|
| **`set-revision-persistence`** | **Active** — architecture locked 2026-06-26 | `/opsx:apply` → **1.1** metadata + catalog |
| `workspace-session-persistence` | M1–M2 + 2.13–2.14 shipped; M3–M4 **superseded** | See `SUPERSEDED.md` |
| `currentset-savedset-storage-layout` | §1 shipped; §2–3 **parked** | See `PARKED.md` |
...
```text
/MidiLooper/current/workspace.bin       # epoch/provenance; interim runtime bundle under temp/
/MidiLooper/current/slots/loop_TT_SS.bin  # 2-digit zero-padded, per dirty slot
/MidiLooper/recovery/checkpoints/       # boot fallback
```
...
M2 `saveNewSet` copies CurrentSet tree (per-slot files). **Hybrid layout change** (`currentset-savedset-storage-layout`) will switch SavedSet to packed **`set.bin` + `loops.bin`** while keeping CurrentSet per-slot.

### Hybrid layout (planned — OpenSpec only)

| Scope | Files | Rationale |
|-------|-------|-----------|
| **CurrentSet** | `workspace.bin` + per-slot `loop_TT_SS.bin` | Incremental dirty-slot writes |
| **SavedSet** | `set.bin` + packed `loops.bin` | Low file count, fast snapshot |
| **Slot index** | Slot-summary rows in `workspace.bin` | Browser/search without opening loop files |
...
### CurrentSet dirty anchor (`workspace.bin`)
...
| **M3** | Slot loop import (long-press → IMPORT LOOP) | **Postponed** — after `currentset-savedset-storage-layout` §2 |
...
| `src/CurrentSetStorage.cpp` | Path helpers, workspace.bin / set.bin schema |
