...
**S0 (shipped 2026-08-12):** `RuntimeTimingEnvelope` + Tier-A `DIAG,msi` / `DIAG,midisvc` / `DIAG,clk` / `DIAG,tracks` / `DIAG,clockrate` (5 s rate limit). **S0b (firmware shipped):** `DIAG,usbdev` / `din` / `hosttask` / `hostdrain` inside `handleMidiInput`. Observation only — no scheduling decisions. Native `test_runtime_timing_envelope` covers both. **Next:** device re-run vs [`191356`](../../captures/session_20260812_191356.log); do not start S1 until a named drain is attributed.
