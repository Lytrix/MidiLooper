...
**S0 (shipped 2026-08-12):** `RuntimeTimingEnvelope` + Tier-A `DIAG,msi` / `DIAG,midisvc` / `DIAG,clk` / `DIAG,tracks` / `DIAG,clockrate` (5 s rate limit). **S0b (attributed [`193645`](../../captures/session_20260812_193645.log)):** PLAYING/OVERDUB `midisvc` is `usbdev`. **S0c (firmware shipped):** `usbread` / `usbdisp` / `usbcap` / `usbthru` inside `usbdev`. Observation only — no scheduling decisions. **Next:** device re-run vs [`193645`](../../captures/session_20260812_193645.log); do not start S1.
