...
**S0 (shipped 2026-08-12):** `RuntimeTimingEnvelope` + Tier-A `DIAG,msi` / `DIAG,midisvc` / `DIAG,clk` / `DIAG,tracks` / `DIAG,clockrate` (5 s rate limit). Observation only — no scheduling decisions. Native `test_runtime_timing_envelope` PASS. **Next:** flash + ≈100-bar device gate; review envelope before S1.
