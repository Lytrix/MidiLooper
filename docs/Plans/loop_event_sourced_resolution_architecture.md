...
1. **6A — prepared display range (idle).** **PASS** [`185931`](../../captures/session_20260815_185931.log) `DIAG,lcr,6a,win=784,proj=5539,oracle=9192,tot=6323,ev=78,notes=40,match=1`. `midi_gap` 43.6 ms during LCR / 14.3 ms after. DFRAME 0.977 s at the sample (idx peak 1.027 s). No `VCACHE,full`. Production idle consume waits for 6B dirty ranges (stamp still matches only until the next commit). **6A.1** [`024806`](../../captures/session_20260816_024806.log): `resolveWindow` unique must not collapse same-tick NoteOffs (`noteId` is ON-only); skip `appendNoteEvents` when the target ON is already in the window. Device `6a` `match=` is LCR vs native (`gatherCommittedEvents` + `filterMidiEventsToWindow`). Gather remaps stay on `6a,nat` `gmatch`.
