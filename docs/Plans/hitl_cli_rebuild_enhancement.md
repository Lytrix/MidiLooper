...
**Staging note:** Part I below is the source for [`docs/Authority/Architecture/HITL_ARCHITECTURE.md`](../Authority/Architecture/HITL_ARCHITECTURE.md). Extract it in Phase 1; contributor and regression guides follow in Phase 4. Archive this plan when migration completes (Phase 5).
...
After migration completes, **major** changes to the long-lived HITL architecture should be introduced through **architectural decision records** (OpenSpec change + append to [`DECISION_LOG.md`](../DECISION_LOG.md)), not by silently editing [`HITL_ARCHITECTURE.md`](../Authority/Architecture/HITL_ARCHITECTURE.md).
...
Migration phases are **scheduling only**. Implementation authority during migration: `openspec/changes/hitl-cli-rebuild/tasks.md` (created Phase 0). When complete, Part I lives in [`HITL_ARCHITECTURE.md`](../Authority/Architecture/HITL_ARCHITECTURE.md); this doc and the OpenSpec change are archived.
...
1. **Extract Part I** → [`docs/Authority/Architecture/HITL_ARCHITECTURE.md`](../Authority/Architecture/HITL_ARCHITECTURE.md).
