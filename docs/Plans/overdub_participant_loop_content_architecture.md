...
**Status:** Phase 1–2a **closed**. **Phase 3** note-on fill disable **in tree**. 1-bar occupy HITL **PASS** [`121933`](../../captures/session_20260818_121933.log). **Phase 4** note-off fill skip when the source view covers the loop **in tree**. 1-bar HITL **PASS** [`123803`](../../captures/session_20260818_123803.log). **Stage 1** membership (span copy past occupy-set cap 128) **shipped**. **Stage 1b** prepared-session length identity **shipped**. **Stage 1c** LCR dirty/save stall **in tree** — [`overdub_participant_64bar_source_view_identity_and_note_off_fill_bugfix.md`](overdub_participant_64bar_source_view_identity_and_note_off_fill_bugfix.md). Stage 2 not started. NOTE_EDIT overlap is a **sibling consumer**. Hydrate Stages 1–5 **not authorized**.  
**Date:** 2026-08-17  
...
After wrap undo, hold fill `from=win` merged into the frozen source view (`merged=1`…`5`, `notes` 19→31) because `rebuildOverdubSourceView` was not called on session undo. A kept the wrap add and gained the restored source. B stayed at 1. **Fix in tree:** wrap undo/redo calls `rebuildOverdubSourceView` (`why=undo` / `why=redo`) when the source view is established. Live discard does not. Native: `test_overdub_session_undo_rebuilds_source_view_to_match_prepared`.

**Device** [`011009`](../../captures/session_20260818_011009.log) — wrap-undo source-view refresh **PASS**. Seven `lcr,src,why=undo` with `sess_undo why=wrap`. After peel, hold `merged=0` and the next `lcr,part` is `a=1` (or `a=0`), not the 010422 `a=2` set. Production consume still A. `late_clk=0`.
...
Idle `StateCheckpoints::rebuild` remains the full-history owner. Wrap publish does not rematerialize.
...
### Open before coding
None. Session-undo inverse is Phase 2a.
...
## Phase 2 — B collect via RC8 hold on NoteSpans

**Invariant:** when prepared is ready, observation B NoteIds at S equal A for the same pitch, including `end < start` pairs that fail `isWrappedLoopNotePair`.

**Owner:** `LoopContentResolution::tryCollectPreparedPresentNoteIdsAtTick`. `Loop::tryCollectPreparedPresentNoteIdsAtTick` is the wrapper. Production consume stays A.

**Not changed:** `notePresentAt`, `tryResolvePreparedState`, idle-gate rebuild, Path B live capture.
...
Named extra at 64 is wrap-crossing Add `10` (then `11`). Do **not** choose “apply all EditPasses at collect” from that pin — the extra is a wrap-local reconstruct row, not a missed Delete. Do not change `notePresentAt`. Do not start Phase 3 fill disable. Idle-gate miss / `from=miss` (013327 session-2 open burst) is a separate class.
...
These populations do **not** produce `a=0,b>0`.

### Native pin (wrap-crossing covers 64)
