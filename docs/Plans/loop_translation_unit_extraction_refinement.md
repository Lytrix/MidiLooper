...
2. **Do not pre-commit to a catch-all module** (`*Maintenance*`, `*Management*`, `*Utilities*`, `*Helpers*`, `*Misc*`). Those names mean the true process owner is not yet identified ([NAMING.md](../Authority/NAMING.md)).
...
**Fifth TU:** no additional translation unit is **currently expected**. If Phase 10 inventory finds a cohesive process that does not fit the four modules, name it for that process ([NAMING.md](../Authority/NAMING.md)) and get user approval for any new domain noun. Do **not** default to `*Maintenance*` / `*Utilities*`.
