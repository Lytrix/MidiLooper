...
**RC-E root cause proven [`162230`](../../captures/session_20260812_162230.log) — display, not storage.** `DisplayManager::refreshViewportAfterOverdubStop` adopts the composed 16-bar display window as the loop's *entire* visual cache, clears `dirtyBars`, and sets `visualCacheDirty = false`. The cache drops 1 042 → 385 notes at the first overdub stop and 1 298 → 304 at the second, coverage collapses to a band around the playhead, and because the cache is declared complete `rebuildVisualCacheIdleSlice` can never backfill it. That is the "piano roll not redrawn after stopping" report. This is shipped RC5c behaviour; the no-gather-on-stop intent is sound, the completeness claim is not. **No content is lost at commit** — `VCACHE,full` never fires, so the earlier content-loss hypothesis is dead. See §31g-2.
**Secondary:** the PLAYING/OVERDUB refill slice is limited to ±20 bars of playhead or loop tail, leaving bars 36–62 of an 84-bar loop unreachable while the transport runs.
