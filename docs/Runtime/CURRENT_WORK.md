...
**RC-H fix shipped:** `OVERDUBBING → STOPPED` keeps the composed overdub-stop frame (`liveOverdubStopHandoffActive_`) instead of letting a clean visual cache replace it with a frozen 16-bar window. [`174843`](../../captures/session_20260812_174843.log): adopted 373 notes, then window-filter 297 at `wStart=7278`. Persistence coalesced (`already_pending`); not a dual-save race. Device verify pending.
