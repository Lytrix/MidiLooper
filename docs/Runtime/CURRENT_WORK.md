...
**Next:** device re-run to confirm the frame period returns to ~30 ms and to re-measure `msi` / `midisvc` with display load halved. Then the resolve budget (gather 25.7 ms, window filter 5.48 ms), then S0b.  
