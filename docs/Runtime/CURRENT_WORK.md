...
**NOTE_EDIT mover wrap-length jump (RC1 native shipped; device gate open):** [`193838`](../../captures/session_20260813_193838.log). Plan: [`note_edit_mover_wrap_length_jump_bugfix.md`](../Plans/note_edit_mover_wrap_length_jump_bugfix.md). `LoopTickNormalize` Pass 1 no longer wrap-merges when both events are tagged and `noteId`s disagree. RC0 fixture now requires 111 end **671** / length **47**. Do not patch `moveNoteWithOverlapHandling`, projection, or `noteEditFocusApplyDisplayNote`. Do not reopen Stage 8/9 rematerialize. Do not call overdub consume.
