...
**Scheduler prep:** [`runtime_scheduler_lcr_consumer_grooming_refinement.md`](../Plans/runtime_scheduler_lcr_consumer_grooming_refinement.md). **Slice 1–4b device PASS.** **Slice 4c:** STOPPED idle always `rebuildVisualCacheIdleSlice` (no `ensureVisualCacheBuilt`). Do not globally delete `ensureVisualCacheBuilt`. Do not fold NOTE_EDIT hydrate into this slice. Do not optimize `LoadLoopJob` from PLAYING paint.
