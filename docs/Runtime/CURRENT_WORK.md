...
Last updated: 2026-08-12 (S0b attributed to `usbdev` in [`193645`](../../captures/session_20260812_193645.log); RC-I `752273d`)
...
**Now: S0b closed.** [`193645`](../../captures/session_20260812_193645.log) attributes PLAYING/OVERDUB `midisvc` to `usbdev` (equals within 0–3 µs). `din` / `hosttask` / `hostdrain` are 1–100 µs. Do not implement admission, change MIDI service density, patch RC-J, or chase overdub display frame-skip. Next observation split (not authorized until confirmed): `usbMIDI.read()` vs `dispatchMidiBatch` / `handleMidiMessage` inside `usbdev`. Full brief: architecture doc [§31i](../Plans/runtime_scheduling_admission_model_architecture.md#31i-run-193645--s0b-attributed-to-usbdev) + [§31c](../Plans/runtime_scheduling_admission_model_architecture.md#31c-s0b--segment-the-midi-service-interval-observation-only).
...
**S0b exit:** **met** in [`193645`](../../captures/session_20260812_193645.log). RECORD `usbdev` 0.7 ms; PLAYING/OVERDUB `usbdev` 78–154 ms matching `midisvc`. Historical 762.5 ms / 129–149 ms / 218–221 ms samples are that same `midisvc` term.
...
**Remaining findings:** S0b named `usbdev`; a split of that drain is the next observation question. RC-S0c still owed (13 `RING,overflow` in 193645; RECORD envelope missing until 126 s). RC-J reproduced at stop (`msi` 273 ms, `midisvc` 3 µs, `clockrate` 0). Display frame-skip / rolling-window during overdub is RC-F follow-up + §31d.
...
**Now:** S0b closed — named drain is `usbdev`. Do not start S1 / admission / RC-J. Next observation split inside `usbdev` only if confirmed.
