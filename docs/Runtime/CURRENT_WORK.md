...
**Overlap-hold stop totals (4-bar confirmed [`162856`](../../captures/session_20260813_162856.log)):** one `#CAP,DIAG,overlap_hold` at overdub-stop seal. Slot 1, 57 notes: Add-only then Shorten/Hide; third stop `empty_sets=3`. `max_lookup_us` 41 / 209 / 319 vs `notechg` 68 / 417 / 601; `noterecon=0`. Each lookup walked the full source view (`max_examined` 57 / 82 / 84). 68-bar slot 4 (`max_same_pitch=322`) is still owed before changing the lookup source.
