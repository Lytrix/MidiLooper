...
Phase 1 **device capture** [`232510`](../../captures/session_20260817_232510.log): 1-bar loop (768). `lcr,part` present. First overdub enter `from=win` then six `from=miss` (`us=2–6`). After wrap 1, `from=prep`. 22/28 prep `eq=1`; 6/28 `eq=0` with `ao=0` `bo≥1`. Pitch 60 at storage tick 528: wrap1 `a=1,b=1`; wrap2 `a=1,b=2`; wrap3 `a=1,b=3`. Source-view `notes=5` across those wraps. Hold `merged=0` every time (`from=win`). `late_clk=0`. Production ids unchanged (`overlap_hold` `max_ids=1`). Do **not** start Phase 2 wrap-predicate unification or Phase 3 fill disable until the extra B ids after wrap are classified. Do not start the Experiment 1 1/2/4/8/16 rebuild series as production policy. Do not delete `overdubSourceView`. NOTE_EDIT overlap is a sibling consumer (selected/mover LinearSpan) — hydrate firmware also not authorized.
