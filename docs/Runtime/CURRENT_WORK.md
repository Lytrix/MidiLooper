...
**NOTE_EDIT mover wrap-length jump (RC0 shipped; RC1 next):** [`193838`](../../captures/session_20260813_193838.log) after `HideNote` 16, mover 111 paints `DNTE` length **2351** and the next bridge uses `focus.last` end **2975**. Plan: [`note_edit_mover_wrap_length_jump_bugfix.md`](../Plans/note_edit_mover_wrap_length_jump_bugfix.md). RC0 fixture: apply leaves 111 at **671**; `LoopTickNormalize::normalize` Pass 1 wrap-merges a same-pitch wrap-window on with 111’s off (no `noteId`) to **2975**; `syncProjectingRowsFromSessionStore` copies it. Do not patch `moveNoteWithOverlapHandling`, projection, or `noteEditFocusApplyDisplayNote`. Do not reopen Stage 8/9 rematerialize. Do not call overdub consume.
