...
**RC-F fix shipped:** clean-cache overdub filter now gathers the paint window widened by `kWindowedGatherMarginBars = 2` and records it, and `committedWindowStale` drops the `visualCacheDirty` term — the committed layer follows auto-follow at ~one filter per two bars instead of freezing. See §31g-3.
