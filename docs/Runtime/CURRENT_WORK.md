...
**Now:** Stages 6–8 PASS. Stage 9 native worst-case µs recorded on the 64-bar fixture (host: materialize 145 µs, 16-bar window 28 µs, checkpoint rebuild 214 µs, high-tick `resolveState` 1 µs). Device probe is blocked: linking `LoopContentResolution.cpp` into `teensy41-capture-serial` overflows RAM1 by ~25 KB ITCM. Production stays on materialize / 3b copy. Do not persist checkpoints (D3).
