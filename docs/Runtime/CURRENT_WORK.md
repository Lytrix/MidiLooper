...
Last updated: 2026-08-18 (004341 O: HITL; prepared miss)
...
**Phase 2a** `76623cd` + **O:** / `sess_undo` CAP `a384a47`. Device [`004341`](../../captures/session_20260818_004341.log): **O:** confirmed. 125 `lcr,part` all `from=miss` — idle gate never finished (`skip,restore` only). Five `sess_undo` `#CAP`, all `why=live`. Do **not** start Phase 3 fill disable until Phase 2 wrap predicates and LinearSpan-from-`NoteSpan` pass. Do not start the Experiment 1 1/2/4/8/16 rebuild series as production policy. Do not delete `overdubSourceView`. NOTE_EDIT overlap is a sibling consumer.
