...
**013327** [`013327`](../../captures/session_20260818_013327.log): 53 `from=prep`, 31 `eq=0` all B extras. Majority `a=0,b>0` at storage **64**. Named extra: wrap-crossing Add. Native `test_prepared_hold_ids_pin_b_extra_wrap_crossing_covers_64`: after wrap 1, A=`[]` B=`[10]`; after wrap 2, B extras `10`,`11`. Class: wrap-local reconstruct of `entry->events` creates a span that covers 64; prepared-window reconstruct of record+wrap does not (015618). Occupy/undo/sibling pins still match. Do **not** start Phase 3 fill disable until hard PLAYING/STOPPED/MUTED/outside-gather identity and prepared HITL `eq=1`. Do not start the Experiment 1 1/2/4/8/16 rebuild series as production policy. Do not delete `overdubSourceView`. NOTE_EDIT overlap is a sibling consumer.
