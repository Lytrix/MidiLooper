...
**Gate 2 (native landed, device open):** enabled slots keep advancing playback; mute/solo/slot-mute gate `MidiHandler` only. Native proves cursor/wrap advance while port is suppressed, unmute does not resend crossed events, and mute does not clear the ledger. Device still owed: mute mid-note silences the port.
