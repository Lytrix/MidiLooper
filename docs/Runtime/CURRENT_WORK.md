...
**RC-E (new, storage/commit — not scheduling):** the visual cache is marked clean while covering ~22 % of the loop. After the second overdub commit `visual` holds 299 notes confined to bars 21–37 of 77; when the playhead returns to tick 0 the paint window shows **1** note and never backfills, because `visualCacheDirty` is false. Storage is not empty there — the `REVT` dump walks a 16th-note grid from tick 0. `visual` also collapses at both commits (977 → 368, then 1 218 → 299), so either the progressive idle slice over-counts or the commit loses content; not yet separated. This is the "piano roll not redrawn after stopping" report. See §31g.
