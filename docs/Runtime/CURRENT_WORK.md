...
**Now:** **C3 device PASS** [`173243`](../../captures/session_20260816_173243.log) — empty-pair overlay skip holds (Pitch resolve 84→**10 ms**). Remaining Layer C: overlay on non-empty pairs (Move `pairs=12` overlay **123 ms**). Do not skip overlay when pairs are non-empty. Do not skip `currentState->clone()`. Do not start B2b / Layer D.
