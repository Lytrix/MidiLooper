...
**RC-F (new, unmasked by the RC-E fix):** committed notes drain out of the piano roll during overdub — `wNotes` falls 295 → 213 as the paint window advances, because `committedWindowStale` requires `visualCacheDirty`, which is now false. Overdub notes themselves are added correctly. Not patched: the naive fix puts a ~4.6 ms window filter on every overdub frame; a wider gather window is the real answer. See §31g-3.
**RC-G (new):** during RECORD the overview strip is fed only the current 16-bar window, so bars outside it are blank; during overdub it uses the clean `visualCache` and shows everything. Frame data is complete (`frame=1238`). Not patched: needs a bounded density histogram, not an O(record-length) scan.
**Remaining findings unchanged:** S0b still dominant (`midisvc` 82.7–128.9 ms vs `clk` ≤ 9.8 ms; `clk` ≈ `tracks`, so dispatch is fully attributed and 79–125 ms stays unattributed inside `handleMidiInput`). Post-stop stall reproduces (131.5 ms). New 324 ms record-stop block with only 9.9 ms in MIDI service. RC-S0c still owed — a 95 s `DIAG` hole during RECORD.
