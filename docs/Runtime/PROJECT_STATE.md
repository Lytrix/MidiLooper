...
Last updated: 2026-08-18 (DEC-041 `Entry.noteId` from `evt.noteId`)

---

## Current branch

**`feature/overdub-participant-discovery`** — [DEC-041](../DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat): Stage 0 identity is `MidiEvent.noteId`. `Entry.noteId` written from `sendMidiEvent`. Occupy lookup still blocked. `evaluateOccupyOverlap` parked. Stage 1c dirty/save stall shipped; Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** ([`123803`](../../captures/session_20260818_123803.log)). Native 1342/1342. PR pending → `dev`.
...
- **Overdub participant discovery:** `feature/overdub-participant-discovery` — DEC-041 `Entry.noteId` (no firmware) — [CURRENT_WORK.md](CURRENT_WORK.md)
...
- **Next product slice:** occupy lookup of `Entry.noteId` — blocked on write-before-emit — [`overdub_present_at_tick_jit_enhancement.md`](../Plans/overdub_present_at_tick_jit_enhancement.md)
