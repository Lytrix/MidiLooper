...
**`feature/overdub-participant-discovery`** — Occupy source-view RC **FROZEN** (5A + sibling D2-D [`174246`](../../captures/session_20260819_174246.log)). Wrap display D2-D **FROZEN**. Consume candidate attribution **Stage 1A/1B shipped**; selection change **REJECTED** on HITL [`191133`](../../captures/session_20260819_191133.log) (non-empty occupy set still needs the window scan). Follow-up HITL [`193024`](../../captures/session_20260819_193024.log): `norow` did not reproduce; `scanadd` persisted. 1B `DIAG,consume,norowid` + `DIAG,consume,scanadd` stay in tree for monitor-only `norow` tagging — [`overdub_consume_ledger_merge_enhancement.md`](../Plans/overdub_consume_ledger_merge_enhancement.md). Native **1386/1386**. RAM1 code **425964** / locals **4768**. 64-bar `from=span` HITL **parked** (DEC-041).
