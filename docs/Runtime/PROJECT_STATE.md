...
Last updated: 2026-08-18 (DEC-041 occupy present-at-S JIT; Stage 1 `from=span` parked)

---

## Current branch

**`feature/overdub-participant-discovery`** — [DEC-041](../DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat) occupy present-at-S JIT **pinned** (no firmware). Stage 1c dirty/save stall shipped; Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** ([`123803`](../../captures/session_20260818_123803.log)). Native 1341/1341. PR pending → `dev`.
...
- **Overdub participant discovery:** `feature/overdub-participant-discovery` — DEC-041 occupy JIT **pinned**; Stage 1 `from=span` HITL parked — [CURRENT_WORK.md](CURRENT_WORK.md)
...
- **Next product slice:** occupy present-at-S JIT (DEC-041; native gold next) — see [CURRENT_WORK.md](CURRENT_WORK.md)
