...
**`feature/overdub-participant-discovery`** — [DEC-041](../DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat): occupy reads `Entry.noteId`. Wrap rebuilds merged playback on the wrap tick before occupy ([`overdub_wrap_playback_rebuild_before_occupy_bugfix.md`](../Plans/overdub_wrap_playback_rebuild_before_occupy_bugfix.md); pin [`152745`](../../captures/session_20260818_152745.log) `n=0 a=1 b=1` 71@656). `playCommittedLoopMidi` writes via `applyPlaybackLedgerEvent` before `sendMidiEvent` may emit. `evaluateOccupyOverlap` parked. Stage 1c dirty/save stall shipped; Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** ([`123803`](../../captures/session_20260818_123803.log)). Native 1345/1345. PR pending → `dev`.
...
- **Overdub participant discovery:** `feature/overdub-participant-discovery` — wrap rebuilds playback before occupy ([`152745`](../../captures/session_20260818_152745.log)) — [CURRENT_WORK.md](CURRENT_WORK.md)
...
- **Next product slice:** wrap rebuilds playback before occupy — [`overdub_wrap_playback_rebuild_before_occupy_bugfix.md`](../Plans/overdub_wrap_playback_rebuild_before_occupy_bugfix.md)
