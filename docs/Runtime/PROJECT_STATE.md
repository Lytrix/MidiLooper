...
**Active work:** **6A same-end Delete** — `resolveWindow` unique keeps same-tick NoteOffs (`noteId` is ON-only); skip `appendNoteEvents` when the target ON is already in the window. Native fixture from [`024806`](../captures/session_20260816_024806.log). Device `6a` `match=` is LCR vs native. **Overdub-pass wrap pairing HITL PASS** [`021218`](../captures/session_20260816_021218.log). **038.2 landed.** Not all of LCR live.
