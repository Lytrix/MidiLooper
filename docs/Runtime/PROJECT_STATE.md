...
Last updated: 2026-08-20 (capture telemetry unsampled-heap stages)
...
**Active work:** Occupy clock same-tick Off-before-On HITL **PASS** [`001021`](../../captures/session_20260819_001021.log) ([`overdub_occupy_clock_same_tick_off_before_on_bugfix.md`](../Plans/overdub_occupy_clock_same_tick_off_before_on_bugfix.md)) — `n=0 a=1` = 0; `n=1 a=0` = 0. Parent catch-up pin [`235314`](../../captures/session_20260818_235314.log) was 4. Pin [`233247`](../../captures/session_20260818_233247.log) was 11. Interval catch-up in tree. Merged-capture gather **FROZEN**. USB occupy `playMidiEvents` catch-up HITL **FAIL** [`214856`](../../captures/session_20260818_214856.log) **reverted**. Loop-head storage-0 occupy HITL **PASS** [`203948`](../../captures/session_20260818_203948.log). Wrap-S catch-up HITL **PASS**. Occupy reads `ActiveNoteLedger` at `currentTick` ([DEC-041](../DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat)). Occupy lookup: `ledger.noteId(midiChannel, pitch)`. Phase 2 **PASS** [`040236`](../captures/session_20260818_040236.log). PLAYING/STOPPED/MUTED HITL **skipped** ([DEC-040](../DECISION_LOG.md#dec-040-skip-playingstoppedmuted-overdub-participant-hitl)). **Phase 4 in tree**. Phase 3 1-bar occupy **PASS** [`121933`](../captures/session_20260818_121933.log). Stage 1 `from=span` HITL **parked**. Do not shrink `kOverdubSourceWindowBars`. Queued (no firmware): [`overdub_loop_length_during_overdub_enhancement.md`](../Plans/overdub_loop_length_during_overdub_enhancement.md). **NOTE_EDIT hydrate** queued ([`note_edit_hydrate_enhancement.md`](../Plans/note_edit_hydrate_enhancement.md)). **Playback gather** Stage 1 hooks landed. Wrap-move persist parked [`201446`](../captures/session_20260816_201446.log).
...
- **Overdub arm pre-roll note capture:** while the record button short-press is pending, `PLAYING` notes are buffered and flushed on `Track::startOverdubbing`; this preserves first note-on capture across the tap-resolution window. Follow-up regression from [`141858`](../../captures/session_20260820_141858.log) fixed: pre-roll flush no longer executes per-note hold snapshot/catch-up on start path (removes multi-second start stall/reboot risk). Follow-up RC from [`142651`](../../captures/session_20260820_142651.log) fixed: overdub-open source-span cache rebuild no longer triggers full `passes.materializeToEventVector` during in-session prepared-span misses, removing the measured `ODUB stage begin_capture` 2,482,421 us stall. Follow-up RC from [`143518`](../../captures/session_20260820_143518.log) fixed: overdub-open fallback no longer allocates source-span cache from `visualCache.notes` while an overdub session is active, avoiding allocator abort/reboot risk seen as `#CAPTURE_RECONNECT` immediately after overdub start. Follow-up RC from [`144819`](../../captures/session_20260820_144819.log) fixed: `TrackManager::startOverdubbingTrack` no longer runs disabled-pass reclaim in the overdub-start window, so large undo stacks are reclaimed only on existing idle/pressure paths instead of during capture start. Follow-up RC from [`145026`](../../captures/session_20260820_145026.log) fixed: deferred slot-restore/load frames are now skipped while capture is active, preventing lazy-load work from sharing the overdub timing-critical window. Follow-up RC from [`145554`](../../captures/session_20260820_145554.log) fixed: overdub stop now uses timing-critical capture flush (`flush=8`) instead of stop-time burst flush (`flush=256`) to prevent serial blocking/overflow during back-to-back overdub transitions. Attempted follow-up from [`150127`](../../captures/session_20260820_150127.log) to remove non-capture track merged-runtime release at overdub start was reverted after regression evidence in [`150427`](../../captures/session_20260820_150427.log): immediate reboot on first overdub start (`Overdubbing started` then `#CAPTURE_RECONNECT` with no stop stage). New RC from [`150844`](../../captures/session_20260820_150844.log), [`151235`](../../captures/session_20260820_151235.log), [`151534`](../../captures/session_20260820_151534.log), and [`151859`](../../captures/session_20260820_151859.log): pre-overdub disabled-pass reclaim bursts (`DIAG,reclaim`) occur before first overdub start and align with the reboot-era captures. `TrackManager::reclaimUnreferencedDisabledPasses` now exits while deferred undo snapshot hydrate is pending, centralizing the guard at the reclaim owner so all reclaim callsites are blocked during hydrate. Follow-up RC from [`152351`](../../captures/session_20260820_152351.log): reboot path is closed (five overdub start-stop cycles complete with no reconnect), but sustained Critical no-op reclaim scans (`passes=0 chunks=0`, ~5.7 ms each) caused capture ring overflow and transport-budget pressure. Main-loop Critical reclaim is now event-driven (Critical entry or worsening chunk headroom) and no longer requests per-call diagnostic visibility. [`153347`](../../captures/session_20260820_153347.log): crash after `Overdubbing started` with no `DIAG,reclaim`; `startOverdubbingTrack` no longer calls `releaseBackgroundPlaybackMergedMidiEventsMemory` (`resetAll` on other PLAYING tracks). Record start still releases; pressure uses `tryReleasePlaybackMergedMidiEventsMemory`. Follow-up from [`154930`](../../captures/session_20260820_154930.log): reconnect moved to post-start (`manager_done` already logged) with sustained zero-heap stop windows. `MidiButtonActions::handleToggleRecord` now restores background playback-runtime release immediately after `Track::stopOverdubbing()`, preserving the start-path removal while restoring pre-removal release behavior on stop. Follow-up from [`160457`](../../captures/session_20260820_160457.log): overdub-start reconnect after `Overdubbing started`; capture start/stop stage telemetry now emits `kUnsampledHeapBytes` instead of live heap reads. Persist admission still queries heap at the persist owner. Native **1397/1397**; HITL pending.
- **Overdub-stop display snapshot stall:** stop-path `SC_DISP_WINDOW` snapshot no longer materializes a temporary filtered note vector; it now emits `windowNoteCount` via a count-only pass (`DisplayWindowUtils::countDisplayNotesInWindow`). Native **1397/1397**; HITL pending from [`141149`](../../captures/session_20260820_141149.log).
...
- **Next product slice:** loop length during overdub queued — [`overdub_loop_length_during_overdub_enhancement.md`](../Plans/overdub_loop_length_during_overdub_enhancement.md)
