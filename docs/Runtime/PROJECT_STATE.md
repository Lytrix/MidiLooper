...
**`feature/overdub-participant-discovery`** — [DEC-041](../DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat): occupy reads `Entry.noteId`. Wrap-tick ledger catch-up applies `lastCommittedPassId()` `(prev, S]` before occupy ([`overdub_wrap_committed_pass_playback_bugfix.md`](../Plans/overdub_wrap_committed_pass_playback_bugfix.md); pin [`180844`](../../captures/session_20260818_180844.log) wrap 15 `n=0 a=1 b=1` 60@416). Parent wrap-rebuild HITL **FROZEN**. `playCommittedLoopMidi` writes via `applyPlaybackLedgerEvent` before `sendMidiEvent` may emit. `evaluateOccupyOverlap` parked. Stage 1c dirty/save stall shipped; Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** ([`123803`](../../captures/session_20260818_123803.log)). PR pending → `dev`.
...
- **Overdub participant discovery:** `feature/overdub-participant-discovery` — wrap-tick ledger catch-up ([`180844`](../../captures/session_20260818_180844.log) wrap 15) — [CURRENT_WORK.md](CURRENT_WORK.md)
...
- **Next product slice:** wrap-tick ledger catch-up from committed pass — [`overdub_wrap_committed_pass_playback_bugfix.md`](../Plans/overdub_wrap_committed_pass_playback_bugfix.md)
