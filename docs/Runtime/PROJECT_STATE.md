...
- **Overdub-stop seal lag optimization:** captures [`163904`](../../captures/session_20260820_163904.log), [`164944`](../../captures/session_20260820_164944.log), and [`165534`](../../captures/session_20260820_165534.log) are stable (0 reconnects), but stop lag remains seal-driven and variable; [`165534`](../../captures/session_20260820_165534.log) records `ODUB,stop,seal` at **30.8 ms**, **66.6 ms**, and **197.2 ms**, with the 197.2 ms spike containing 25 `DIAG,seal_companion` rows in that same cycle. Owner path remains companion sealing (`Loop::applyPendingHideAndShortenToNotes` + `Loop::sealPendingNoteChangesToEditPasses` + `Loop::saveNoteEditPass`). Stage now batches source-note transforms, batches derived invalidation to one publish per stop, batches heap-reserve admission to one check per companion seal, and reserves companion edit-pass capacity before row insertion; adds `DIAG,seal_companion_batch` timing. Native **1397/1397** and `teensy41-capture-serial` build pass (`RAM1 code 425020 / locals 4768`); waiting on HITL re-measure with reserve stage — [CURRENT_WORK.md](CURRENT_WORK.md)
