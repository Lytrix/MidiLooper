...
Last updated: 2026-08-18 (DEC-040 skip PLAYING/STOPPED/MUTED HITL; Phase 3 next)

---

## Current branch

**`feature/overdub-participant-discovery`** — 36 commits ahead of `dev` (`be0e05f`). Tip `f5a7864`. Overdub participant Phase 2 **closed** ([`040236`](../../captures/session_20260818_040236.log)); **Phase 3** fill disable next (firmware not started). Native **1329/1329**. PR pending → `dev`.
...
**Active work:** Overdub participants = notes **present** at tick `S` — wrap-undo source-view refresh HITL **PASS** [`011009`](../captures/session_20260818_011009.log). Phase 2 B collect uses `displayNotePresentAtHold` on prepared `NoteSpan`s; `notePresentAt` unchanged. [`021716`](../captures/session_20260818_021716.log) pre-wrap `a=0` is finished opens on prepared spans. Source-view fill from those spans is in tree (`b94bd2b`) — [`overdub_participant_source_view_span_membership_bugfix.md`](../Plans/overdub_participant_source_view_span_membership_bugfix.md). Device [`024225`](../captures/session_20260818_024225.log) `from=span,notes=0` wiped RC12 display. [`025337`](../captures/session_20260818_025337.log) display wipe **PASS**. [`025916`](../captures/session_20260818_025916.log) / [`030219`](../captures/session_20260818_030219.log) storage 64 before wrap is `from=miss` on a 1-note loop. 030219 wrap 1 `from=span,notes=34` vs `vch notes=4` — span copy now window-noteId only. [`030958`](../captures/session_20260818_030958.log) flood **PASS** (wrap 1 `notes=14` = `vch 14`). [`032228`](../captures/session_20260818_032228.log) flood **PASS** (7 wraps delta 0). Wrap 6–7 `bo=1` is B overcount; B collect now skips Disabled-pass / superseded companion restore. Device [`034455`](../captures/session_20260818_034455.log) **PASS** (`eq=0` = 0). Wrap 1-frame undo flash: span copy uses the same Disabled-companion restore guards as B collect; session undo/redo invalidates live display cache. Device [`040236`](../captures/session_20260818_040236.log) **PASS**. 021716 is not a blocking gate. PLAYING/STOPPED/MUTED participant HITL **skipped** ([DEC-040](../DECISION_LOG.md#dec-040-skip-playingstoppedmuted-overdub-participant-hitl)). **Phase 3 next** (firmware not started) — [`overdub_participant_loop_content_architecture.md`](../Plans/overdub_participant_loop_content_architecture.md). Experiment 1 source/hold detach (`kOverdubSourceWindowBars` = 16) is in tree; do not treat a smaller window as the next firmware. Display undo/wrap/pitch-move — RC-W1/U1 HITL PASS [`152627`](../captures/session_20260817_152627.log); [`155450`](../captures/session_20260817_155450.log) NOTE_EDIT exit notes **5** — [`note_edit_display_undo_overdub_wrap_bugfix.md`](../Plans/note_edit_display_undo_overdub_wrap_bugfix.md). **Shipped on device:** wrap-crossing overdub consume `e1f57c5`, HITL **PASS** [`155450`](../captures/session_20260817_155450.log) — [`overdub_wrap_crossing_hold_head_consume_bugfix.md`](../Plans/overdub_wrap_crossing_hold_head_consume_bugfix.md). Overdub occupied-lane RC11/RC12 **FROZEN** HITL PASS [`140355`](../captures/session_20260817_140355.log) — [`overdub_overlap_hold_display_cache_bugfix.md`](../Plans/overdub_overlap_hold_display_cache_bugfix.md). Queued (no firmware): [`overdub_loop_length_during_overdub_enhancement.md`](../Plans/overdub_loop_length_during_overdub_enhancement.md). Parent [`overdub_overlap_hold_same_start_bugfix.md`](../Plans/overdub_overlap_hold_same_start_bugfix.md). RC10 live Hide paint; RC9 transport-stop pending; RC8 hold-window JIT; RC7 enter rebuild; RC6 wrap rebuild. RC1/RC3 device-confirmed. NOTE_EDIT persist close-keep **PASS** [`191411`](../captures/session_20260816_191411.log). Open wrap select **PASS** [`195050`](../captures/session_20260816_195050.log). **Wrap-move persist parked** [`201446`](../captures/session_20260816_201446.log): live linear `2832–3408` / `DNTE` **576**; deselect shortened to **336**. That RC is partly the current rematerialize / session-store structure — do not add more LIFO/wrap-off persist patches. Re-evaluate after hydrate. **LoopPersist finalize CRC relanded**. Stage 2 parked. Parent [`note_edit_undo_warm_missing_recon_investigation.md`](../Plans/note_edit_undo_warm_missing_recon_investigation.md). **C1–C4 device PASS**; **C5 parked**. **B2a device PASS** [`171228`](../captures/session_20260816_171228.log). A intermediates PASS. Boot title RAM1 **closed**. LED lookup Stage 1 PASS [`114736`](../captures/session_20260816_114736.log). Consumer grooming Slice 1–4c device PASS; 4e parked. **Slice 1b device PASS [`225626`](../captures/session_20260816_225626.log)** — `idle_append` owns dirty `idle_maint`. **Slice 2b device PASS [`232423`](../captures/session_20260816_232423.log).** **Slice 2c device PASS [`233323`](../captures/session_20260816_233323.log).** **038.2 landed.** **6C/6D closed** [`205928`](../captures/session_20260815_205928.log) / [`210508`](../captures/session_20260815_210508.log) (3b stays; not all of LCR live). **NOTE_EDIT hydrate** queued ([`note_edit_hydrate_enhancement.md`](../Plans/note_edit_hydrate_enhancement.md) — overlap = selected/mover LinearSpan participants, not a `selectedTick` window); **playback gather** queued ([`playback_gather_lcr_consume_enhancement.md`](../Plans/playback_gather_lcr_consume_enhancement.md)); wrap-move is not a hydrate start gate.
...
- **Overdub participant discovery:** `feature/overdub-participant-discovery` — Phase 2 **closed** ([DEC-040](../DECISION_LOG.md#dec-040-skip-playingstoppedmuted-overdub-participant-hitl)); Phase 3 next — [CURRENT_WORK.md](CURRENT_WORK.md)
...
- **Next product slice:** overdub participant Phase 3, then HITL CLI Phase 3 or persistence overlay — see [CURRENT_WORK.md](CURRENT_WORK.md)
