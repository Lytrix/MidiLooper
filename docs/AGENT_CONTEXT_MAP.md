...
- [plans/overdub_participant_loop_content_architecture.md](Plans/overdub_participant_loop_content_architecture.md) — overdub participants from notes present at tick `S`; `PresentNote` C++ type; NOTE_EDIT overlap sibling (selected/mover LinearSpan); not MIDI send / mute; Phase 1 + companion publish HITL; Phase 2a session-undo inverse planned; hydrate firmware not authorized
