...
**Overdub:** wrap continuation is also off. Session wrap extracts only the unpaired held ON (`OpenNoteOn.eventIndex`, append order) and re-appends it after `beginCapture`. A completed wrap-held pair (`ON` in the tail + `OFF` at tick 0) stays in the sealed wrap even when the same pitch is replayed later ([`012925`](../../captures/session_20260816_012925.log) note 30 ON@2976 / OFF@0 / later 672). Tick-0 Off may wrap-pair despite later body Ons. Live overlay extends to the playhead only. Sealed wrap-held heads still appear on NoteOff via the capture-preview wrap path.
