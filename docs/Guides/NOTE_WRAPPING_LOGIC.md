...
**Overdub:** wrap continuation is also off. Session wrap extracts only preview-open held ONs and re-appends them after `beginCapture`. A completed wrap-held pair stays in the sealed overdub wrap ([`014937`](../../captures/session_20260816_014937.log) note 12 ON@2976 / OFF@96). Merged reconstruct pairing stays on the original `allLaterOnsInTailOrNone` rule — relaxing it on record+overdub together rewrote record-pass lengths around wrap ([`015618`](../../captures/session_20260816_015618.log)). Visual cache appends missing notes from each overdub pass reconstructed with `overdubPassWrapPairing` (`Loop::appendOverdubPassDisplayNotes`) on the unprepared gather path and on prepared idle slices that keep bar 0 or the last bar. Prepared interior idle slices use the LCR window only — merged reconstruct cannot take wrap pairing ([`015618`](../../captures/session_20260816_015618.log)). Device PASS [`021218`](../../captures/session_20260816_021218.log): `WRAP,2976,96,4,12`, VCACHE 63→87. Live overlay extends to the playhead only. Sealed wrap-held heads still appear on NoteOff via the capture-preview wrap path.
