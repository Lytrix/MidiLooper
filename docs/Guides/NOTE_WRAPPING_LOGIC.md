...
**Overdub:** wrap continuation is also off. Session wrap extracts only the unpaired held ON (`OpenNoteOn.eventIndex`) and re-appends it after `beginCapture`. A completed wrap-held pair (`ON` in the tail + `OFF` in the head window) stays in the sealed wrap even when the same pitch is replayed later ([`014937`](../../captures/session_20260816_014937.log) note 12 ON@2976 / OFF@96 / later 288). Only an unpaired body On blocks that wrap pair. Live overlay extends to the playhead only. Sealed wrap-held heads still appear on NoteOff via the capture-preview wrap path.
