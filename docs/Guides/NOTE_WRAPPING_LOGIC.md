...
**Overdub:** wrap continuation is also off. Session wrap extracts only preview-open held ONs and re-appends them after `beginCapture`. A completed wrap-held pair stays in the sealed overdub wrap ([`014937`](../../captures/session_20260816_014937.log) note 12 ON@2976 / OFF@96). Do not change merged reconstruct pairing — that rewrote record-pass lengths around wrap ([`015618`](../../captures/session_20260816_015618.log)). Live overlay extends to the playhead only. Sealed wrap-held heads still appear on NoteOff via the capture-preview wrap path.
