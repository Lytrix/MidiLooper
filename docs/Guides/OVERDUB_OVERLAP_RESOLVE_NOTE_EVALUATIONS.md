...
**Candidate selection (non-empty ids).** When `overlapNoteIds` is non-empty, `accumulatePendingNoteChangesForIncomingNote` builds `effectiveOverlapNoteIds`: occupy/playback ids, geometric participants from `overdubSourceViewNotes_` that `existingNoteOverlapsIncomingHold` with the incoming hold, and hold-segment `ensureOverdubSourceNotesForHold` when rows are missing. Candidates come from `appendNotesForIds` only — the window-scan path in `collectConsumeWindow` runs **only** when the effective set is empty (empty-ids hide contract). Plan: [`../Plans/overdub_consume_id_resolution_completeness_bugfix.md`](../Plans/overdub_consume_id_resolution_completeness_bugfix.md).

**Candidate attribution (observability).** Counters `scanOnlyCandidates`, `lateNoteCandidates`, and `idsWithoutNotes` remain on `OverlapHoldTotals`. After id-resolution completeness, non-empty holds should show `scan=0`, `late=0`, `norow=0` on `DIAG,consume,select`. Empty-set holds may still show `scan>0`. Prior attribution-only plan: [`../Plans/overdub_consume_ledger_merge_enhancement.md`](../Plans/overdub_consume_ledger_merge_enhancement.md) (FROZEN).
