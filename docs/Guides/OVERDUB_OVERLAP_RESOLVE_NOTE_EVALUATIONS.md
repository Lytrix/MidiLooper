...
**Candidate selection.** `accumulatePendingNoteChangesForIncomingNote` builds `effectiveOverlapNoteIds`: occupy/playback ids plus geometric participants from `overdubSourceViewNotes_` that `existingNoteOverlapsIncomingHold` with each hold segment. Long loops call `ensureOverdubSourceNotesForHold` per segment before the geometric pass. Candidates come from `appendNotesForIds` only — `collectConsumeWindow` is **removed** (Stages 1–2). Plan: [`../Plans/overdub_consume_id_resolution_completeness_bugfix.md`](../Plans/overdub_consume_id_resolution_completeness_bugfix.md).
