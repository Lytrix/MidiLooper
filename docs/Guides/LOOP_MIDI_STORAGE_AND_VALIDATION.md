...
| **`overdubSourceView` + pending delta** | At overdub start: materialize-aware baseline. Completed notes → Add/Shorten/Hide via `resolveConstrainedGeometry` (shared `noteMinLengthTicks`). Stop seals Shorten/Hide as EditPass companions; one `OverdubPassAdded` undo (`passIds` = session wraps, GUS STK3; legacy STK2 is a single `passId`). OpenSpec: `overdub-pass-overlap-resolution` (DEC-031/032); DEC-038 038.2 |
