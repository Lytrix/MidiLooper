...
**Gate before firmware:** native 6D.1 **measured 2026-08-15.** Append+merge matches the oracle. Full `tickEvents` sort **and** ordered merge **FAIL** scaling: `index_order_us` grows with `history_events` at fixed delta (sort 602→2624 µs, merge 46→174 µs at 8192→32768). Both < 50 ms. **6D.2 native PASS** the same day: frozen `tickEvents` + delta `TickEventEntryVec`; two-source `findRawWindow` without compacting; `no_delta` `query_us=4` / `candidate_history=8` at H=8192→65536, Δ=8. **6D.3 native PASS** the same day: N=1/4/16 overdubs at H=8192 and 32768, δ=8; history stays frozen; `no_delta` stays 8+0 / `query_us=4`; `commit_us=1` at N=16 on both H. Not authorization to make LCR incrementally live. A production architecture gate may now be considered. Production architecture stays untouched until that gate. `< 3 ms` is consume of already-prepared state ([`045556`](../captures/session_20260814_045556.log) 2214 µs copy), not a promise that switching to LCR is 2.2 ms.
