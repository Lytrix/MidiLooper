...
**Next (persistence/runtime):** DEC-037 **LoopContentResolution prototype** — **6A firmware** (keep prepared `TickIndex`; idle visual slice consumes `resolveWindow`; gather stays oracle). Device `DIAG,lcr,6a` measure owed. Then **6B** commit invalidation → **6C** overdub source (3b copy stays). Score 6C vs 3b [`045556`](../captures/session_20260814_045556.log) **2214 µs**. No `openOnByPitch` flatten / B. Overlay track A parked.
