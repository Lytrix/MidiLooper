...
**Next (persistence/runtime):** **`feature/overdub-participant-discovery`** — wrap rebuilds playback before occupy ([`overdub_wrap_playback_rebuild_before_occupy_bugfix.md`](Plans/overdub_wrap_playback_rebuild_before_occupy_bugfix.md); [`152745`](../captures/session_20260818_152745.log) `n=0 a=1 b=1` 71@656). Occupy lookup of `Entry.noteId` shipped. Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** [`123803`](../captures/session_20260818_123803.log). Loop length during overdub queued. **NOTE_EDIT hydrate** queued. **Playback gather** Stage 1 hooks. Overlay track A parked.
