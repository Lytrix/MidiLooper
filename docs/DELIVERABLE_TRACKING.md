...
**Next (persistence/runtime):** **`feature/overdub-participant-discovery`** — Stage 1c LCR dirty/save stall **in tree** (matching dirty skips; save is not a defer). Stage 1b length identity shipped; Stage 1 membership shipped; [`125542`](../captures/session_20260818_125542.log) `a=1,b=0` = 0. [`131207`](../captures/session_20260818_131207.log) never reached `lcr,mat`. Device HITL: stay STOPPED until 64-bar `lcr,mat` then overdub. Phase 4 1-bar HITL **PASS** [`123803`](../captures/session_20260818_123803.log). Do not start Stage 2 until 64-bar `from=span` / `ao=0`. Parent [`overdub_participant_64bar_source_view_identity_and_note_off_fill_bugfix.md`](Plans/overdub_participant_64bar_source_view_identity_and_note_off_fill_bugfix.md). Loop length during overdub queued. **NOTE_EDIT hydrate** queued. **Playback gather** Stage 1 hooks. Overlay track A parked.
