...
| MVP: Memory expansion (PSRAM/OSRAM) to increase undo + loop capacity (>= 11000 notes) | `TrackUndo`, `Utils/MemoryPool` / `PooledMidiEventVector`, `Track`/`Loop` MIDI event storage | `plans/mvp_psram_memory_expansion_a1356995.plan.md` | Completed (Implemented ExtMemAllocator with PSRAM spillover) |
