...
**Next (persistence):** DEC-035 Layer A — Loop persists content only; delete `UndoStacks` after load-time editing-state reconstruction. Plan: [`loop_layer_history_persistence_architecture.md`](Plans/loop_layer_history_persistence_architecture.md). OpenSpec `loop-content-history-persistence`.
