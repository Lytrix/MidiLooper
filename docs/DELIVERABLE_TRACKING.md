...
**Next (persistence/runtime):** **`feature/overdub-participant-discovery`** — [DEC-041](DECISION_LOG.md#dec-041-occupy-present-at-s-jit-not-full-loop-lcr-mat) occupy lookup of `Entry.noteId`. Plan [`overdub_present_at_tick_jit_enhancement.md`](Plans/overdub_present_at_tick_jit_enhancement.md). Stage 1c dirty/save stall shipped; Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** [`123803`](../captures/session_20260818_123803.log). Loop length during overdub queued. **NOTE_EDIT hydrate** queued. **Playback gather** Stage 1 hooks. Overlay track A parked.
