...
**Next (persistence/runtime):** **`feature/overdub-participant-discovery`** — wrap-tick ledger catch-up ([`overdub_wrap_committed_pass_playback_bugfix.md`](Plans/overdub_wrap_committed_pass_playback_bugfix.md); [`180844`](../captures/session_20260818_180844.log) wrap 15 `n=0 a=1 b=1` 60@416). Occupy lookup of `Entry.noteId` shipped. Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** [`123803`](../captures/session_20260818_123803.log). Loop length during overdub queued. **NOTE_EDIT hydrate** queued. **Playback gather** Stage 1 hooks. Overlay track A parked.
