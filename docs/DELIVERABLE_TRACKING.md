...
**Next (persistence/runtime):** **`feature/overdub-participant-discovery`** — wrap-S HITL **PASS** [`185831`](../captures/session_20260818_185831.log); loop-head ledger investigation ([`overdub_loop_head_playback_ledger_investigation.md`](Plans/overdub_loop_head_playback_ledger_investigation.md) 60 @ 0). Occupy lookup of `Entry.noteId` shipped. Stage 1 `from=span` HITL **parked**. Do not wait STOPPED for 64-bar `lcr,mat`. Phase 4 1-bar HITL **PASS** [`123803`](../captures/session_20260818_123803.log). Loop length during overdub queued. **NOTE_EDIT hydrate** queued. **Playback gather** Stage 1 hooks. Overlay track A parked.
