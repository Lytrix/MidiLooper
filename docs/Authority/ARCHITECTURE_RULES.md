...
- **Undo routing:** While NOTE_EDIT active, `handleUndo` is session-gated (**E:** only — no **U:** fallthrough). While OVERDUBBING, the same gate uses the overdub session stack (no **U:** fallthrough). After exit, **U:** via global stack (`RecordPassAdded` / `OverdubPassAdded` / `NoteEditPassClosed` / `LoopBoundaryChange` / `ClearSlot` at cursor). One `OverdubPassAdded` after stop carries session wrap `passIds` (DEC-038 038.2). After reboot, `deriveContentUndoUnits` rebuilds that same unit from `overdubSessionIndex`. `NoteEditPassClosed` from `markCurrentEditBatchDurable` is **U:**-reachable only after NOTE_EDIT ends. Clear-slot undo uses `handleUndoClearTrack` when top entry is **ClearSlot**.
